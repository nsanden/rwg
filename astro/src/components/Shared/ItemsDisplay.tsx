...
interface ItemsDisplayProps {
    ...
    onReset?: () => void;  // Optional reset handler for when no matches found (should also regenerate)
}
...
export default function ItemsDisplay({
    ...
    // Calculate font size based on text length - used for initial render and useEffect fine-tuning
    const calculateFontSize = (item: string | undefined, size: 'small' | 'medium' | 'large') => {
        if (!item) return size === 'small' ? 40 : size === 'medium' ? 60 : 80;

        const length = item.length;
        if (length > 18) return size === 'small' ? 16 : size === 'medium' ? 20 : 28;
        if (length > 16) return size === 'small' ? 18 : size === 'medium' ? 24 : 32;
        if (length > 14) return size === 'small' ? 20 : size === 'medium' ? 28 : 36;
        if (length > 12) return size === 'small' ? 24 : size === 'medium' ? 32 : 44;
        if (length > 10) return size === 'small' ? 28 : size === 'medium' ? 40 : 52;
        if (length > 8) return size === 'small' ? 32 : size === 'medium' ? 48 : 60;
        if (length > 6) return size === 'small' ? 36 : size === 'medium' ? 54 : 68;
        return size === 'small' ? 40 : size === 'medium' ? 60 : 80;
    };

    // Fine-tune font size based on actual container width (adjusts down if needed)
    useEffect(() => {
        if (wordRef.current && displayItems && displayItems[0]) {
            ...
            if (container) {
                ...
                // Start with calculated size based on text length
                let fontSize = calculateFontSize(item, textSize);
                wordRef.current.style.fontSize = fontSize + 'px';

                // Adjust down if it doesn't fit in the container
    ...
    const getFontSize = () => {
        const item = displayItems?.[0];
        return `${calculateFontSize(item, textSize)}px`;
    };
            ...
            <div className="flex justify-end gap-2 flex-wrap mb-4 relative z-10">
                {favorites?.length > 0 && (
                    <button
                        ...
                        className={`group flex items-center gap-1 text-sm px-3 py-2 rounded ${showFavorites ? 'bg-red-100 text-red-600' : 'text-gray-600 hover:text-red-600 hover:bg-gray-100'} transition-colors cursor-pointer`}
                ...
                {(displayItems.length > 0 || (showFavorites && favorites.length > 0)) && (
                    <button
                        onClick={copyToClipboard}
                        className="text-gray-600 hover:text-blue-600 hover:bg-gray-100 transition-colors p-2 rounded touch-manipulation cursor-pointer"
            ...
            <div className={`flex-1 flex items-center ${displayItems.length === 1 && !showFavorites ? '-translate-y-8' : ''}`} style={{ pointerEvents: 'none' }}>
                <div className="w-full" style={{ pointerEvents: 'auto' }}>
                    {loading && displayItems.length === 0 ? (
                        // Initial load - show nothing (blank space)
                        <div className="py-8" />
                    ) : loading ? (
                        // Regenerating with existing items - show spinner
                        <div className="text-center py-8" role="status" aria-live="polite">
                            <RefreshCw className="w-8 h-8 text-gray-500 animate-spin mx-auto" aria-hidden="true" />
                        </div>
                    ) : displayItems.length === 0 && !showFavorites ? (
                        <div className="text-center py-8" role="status" aria-live="polite">
                            <p className="text-gray-500 mb-4">No matches found. Try adjusting your filters.</p>
                            {onReset && (
                                <button
                                    onClick={onReset}
                                    className="px-4 py-2 text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md transition-colors cursor-pointer"
                                >
                                    Reset Filters
                                </button>
                            )}
                                    ...
                                    <div className="text-center">
                                        <div className="flex items-baseline justify-center gap-3 w-full">
                                            ...
                                            <button
                                                ...
                                                className="text-gray-400 hover:text-red-500 transition-colors self-center touch-manipulation cursor-pointer"
                                                aria-label={(() => {
                                                        ...
                                                        <div className="flex items-center gap-2 ml-3">
                                                            {!showFavorites ? (
                                                                <button
                                                                    ...
                                                                    className="text-gray-400 hover:text-red-500 transition-colors touch-manipulation flex-shrink-0 cursor-pointer"
