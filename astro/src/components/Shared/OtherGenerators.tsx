...
export default function OtherGenerators({ currentPage = '/', basePath = '' }: OtherGeneratorsProps) {
    const generators = [
        ...
        { href: '/bible.php', label: 'Random Bible Verses' },
        ...
        <div>
            ...
            <ul className="space-y-2">
                {generators.map((generator) => {
                    // Normalize for comparison - strip .php from both and compare
                    const normalizedHref = generator.href.replace(/\.php$/, '');
                    const normalizedCurrent = currentPage.replace(/\.php$/, '');
                    const isCurrentPage = normalizedHref === normalizedCurrent;
